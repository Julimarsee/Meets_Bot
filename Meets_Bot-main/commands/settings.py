...
class SettingsCommand(Command):
    ...
    def execute_command(self, message: Message):
        self.bot.send_text_file(message, 'Meets_Bot-Main/messages/settings.md')
    ...
    def process_settings_choice(self, message: Message):
        try:
            ...
            if choice == "Изменить анкету":
                ...
                self.bot.telebot.send_message(chat_id, "Напишите вашу жалобу:", reply_markup=ReplyKeyboardRemove)
                ...
                if self.db.delete_user(chat_id):
                    self.bot.telebot.send_message(chat_id, "Ваша анкета успешно удалена.", reply_markup=ReplyKeyboardRemove)
                else:
                    self.bot.telebot.send_message(chat_id, "Ошибка при удалении анкеты. Попробуйте снова.")
            elif choice == "Премиум":
                if self.db.premium_user(chat_id):
                    self.bot.telebot.send_message(chat_id, "Премиум-статус успешно активирован!", reply_markup=ReplyKeyboardRemove)
                else:
    ...
    def process_complaint(self, message: Message):
        try:
            ...
            self.bot.telebot.send_message(chat_id, "Ваша жалоба отправлена администратору. Спасибо за обратную связь!", reply_markup=ReplyKeyboardRemove)
        except Exception as e:
