...
class GoCommand(Command):
    ...
    def send_search_interests_message(self, message: Message):
        self.bot.telebot.send_message(message.chat.id, read_text_file('Meets_Bot-Main/messages/search-interests.md'), reply_markup=InterestsKeyboardFactory.get('interests'))
    ...
    def execute_command(self, message: Message, retry=False):
        ...
        if self.db.check_user_exists(chat_id) and not retry:
            self.bot.send_text_file(message, 'Meets_Bot-Main/messages/exists.md')
            self.send_search_interests_message(message)
    ...
    def search_people(self, message: Message):
        try:
            ...
            if message.text.lower() == "закончить ввод":
                self.bot.telebot.register_next_step_handler(message, self.handle_yes_no_response(message, stop=False))
                return
            ...
            matching_users = []
            for interest in current_user.get('interests', []):
    ...
    def handle_yes_no_response(self, message: Message, stop=True):
        try:
            chat_id = message.chat.id
            if message.text == 'Да':
                self.execute_command(message, retry=True)
            elif message.text == 'Нет' or not stop:
                self.bot.telebot.send_message(chat_id, "Поиск завершен.", reply_markup=ReplyKeyboardRemove)
            else:
                self.bot.telebot.send_message(chat_id, "Неизвестная команда. Пожалуйста, выберите 'Да' или 'Нет'.")
        except Exception as e:
